...
describe('parseToSvg', () => {
  ...
  describe('icon 类型', () => {
    it('basic 图形正常解析', async () => {
      ...
      expect(svg.toSketchJSON()).toStrictEqual(svg.toSketchJSON()); // 为什么不相等?
    });
    it('up-circle 图形正常解析', async () => {
      ...
      console.log('源码=>',JSON.stringify(originJSON).length, originJSON)
      console.log('测试=>',JSON.stringify(targetJSON).length, targetJSON)
