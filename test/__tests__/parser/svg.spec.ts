...
describe('parseToSvg', () => {
  beforeAll(() => {
    ...
    `;
    setupTestNode(innerHTML, 'svgId');
  });
  afterAll(()=>{
    removeTestNode("svgId")
  })
  describe('icon 类型', () => {
    it('basic 图形正常解析', async () => {
      ...
      // @todo
      // 63 ？
      // console.log('源码11=>', svg.toSketchJSON().layers[0].resizingConstraint, svg.toSketchJSON().layers[1].resizingConstraint)
      // 27 ？
      // console.log('测试11=>', basicParserJSON.layers[0].resizingConstraint,basicParserJSON.layers[1].resizingConstraint)
